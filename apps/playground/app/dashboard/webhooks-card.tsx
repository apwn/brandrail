...
export function WebhooksCard() {
  ...
  return <section id="webhooks" className="panel p-5 mt-4">
    ...
    <div className="mt-4 flex gap-2"><label htmlFor="webhook-url" className="sr-only">Webhook endpoint URL</label><input id="webhook-url" className="field !py-2" type="url" placeholder="https://automation.example.com/brandrail" value={url} onChange={(event) => setUrl(event.target.value)} /><button className="btn !py-2 whitespace-nowrap" onClick={add} disabled={!url || events.length === 0}>+ Add endpoint</button></div>
    {error && <p className="mt-2 font-mono text-xs text-signal" role="alert">{error}</p>}
