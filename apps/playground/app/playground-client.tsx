...
export default function Playground({ initialUrl = "" }: { initialUrl?: string }) {
  const [step, setStep] = useState<Step>(initialUrl ? "compiling" : "landing");
  const [url, setUrl] = useState(initialUrl);
  const initialCompileStarted = useRef(false);
  ...
  useEffect(() => {
    if (!initialUrl || initialCompileStarted.current) return;
    initialCompileStarted.current = true;
    void doCompile();
  }, [doCompile, initialUrl]);
