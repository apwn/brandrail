...
export function CalendarWorkspace({ initialPosts, channels, renders }: { initialPosts: ScheduledPost[]; channels: Channel[]; renders: SavedRender[] }) {
  ...
  return <main className="mx-auto max-w-[1400px] px-4 py-8 sm:px-6">
    <header className="flex flex-wrap items-end justify-between gap-5"><div><a href="/dashboard" className="eyebrow hover:text-bone">← WORKSPACE</a><p className="eyebrow text-signal mt-6">PUBLISHING RAIL</p><h1 className="font-display text-4xl font-bold mt-2">Content calendar</h1><p className="text-muted text-sm mt-2">Drag scheduled posts between days. The publishing time stays intact.</p></div><button className="btn" onClick={() => setComposer(true)}>+ Schedule content</button></header>
    {error && <div className="panel border-signal/50 px-4 py-3 mt-6 text-sm text-signal">{error}<button className="float-right" onClick={() => setError(null)}>×</button></div>}
...
function Composer({ channels, renders, onClose, onCreated }: { channels: Channel[]; renders: SavedRender[]; onClose: () => void; onCreated: (post: ScheduledPost) => void }) {
  ...
  return <div className="fixed inset-0 z-50 flex items-center justify-center bg-ink/90 p-4"><div className="panel w-full max-w-xl p-6"><div className="flex items-center justify-between"><div><p className="eyebrow text-signal">NEW CALENDAR ITEM</p><h2 className="font-display text-2xl font-bold mt-2">Schedule content</h2></div><button onClick={onClose} className="text-muted hover:text-bone">×</button></div>{channels.length === 0 ? <p className="text-muted mt-6">Connect a publishing channel in the workspace first.</p> : <><textarea autoFocus className="field mt-6 min-h-32" placeholder="Write the post caption…" value={text} onChange={(e) => setText(e.target.value)} /><input className="field mt-2" type="datetime-local" value={when} onChange={(e) => setWhen(e.target.value)} /><div className="mt-4"><p className="eyebrow text-bone mb-2">CHANNELS</p><div className="flex flex-wrap gap-2">{channels.map((channel) => <label key={channel.id} className={`cursor-pointer border px-3 py-2 text-xs ${selectedChannels.includes(channel.id) ? "border-signal text-bone" : "border-hairline text-muted"}`}><input className="sr-only" type="checkbox" checked={selectedChannels.includes(channel.id)} onChange={() => setSelectedChannels((ids) => ids.includes(channel.id) ? ids.filter((id) => id !== channel.id) : [...ids, channel.id])} />{channel.platform} · {channel.handle}</label>)}</div></div><label className="eyebrow text-bone block mt-4">OPTIONAL SAVED CREATIVE<select className="field mt-2 normal-case tracking-normal" value={renderId} onChange={(e) => setRenderId(e.target.value)}><option value="">Text only</option>{renders.map((render) => <option key={render.id} value={render.id}>{render.manifest.brand} · {render.manifest.brief.slice(0,55)}</option>)}</select></label>{error && <p className="font-mono text-xs text-signal mt-3">{error}</p>}<button className="btn w-full mt-5" disabled={busy || !text.trim() || selectedChannels.length === 0} onClick={() => void submit()}>{busy ? "Scheduling…" : "Add to calendar"}</button></>}</div></div>;
}

function CalendarSummary({ posts, channels }: { posts: ScheduledPost[]; channels: Channel[] }) { const scheduled = posts.filter((post) => post.status === "scheduled").length; const published = posts.filter((post) => post.status === "published").length; return <><p className="eyebrow text-bone">THIS WORKSPACE</p><div className="grid grid-cols-2 gap-2 mt-4"><Stat value={scheduled} label="scheduled" /><Stat value={published} label="published" /></div><p className="eyebrow text-bone mt-6">CONNECTED CHANNELS</p><div className="mt-3 space-y-2">{channels.length ? channels.map((channel) => <div key={channel.id} className="border border-hairline px-3 py-2 text-xs"><span className="text-signal uppercase">{channel.platform}</span> · {channel.handle}</div>) : <a className="text-sm text-signal" href="/dashboard#channels">Connect a channel →</a>}</div><p className="font-mono text-[10px] leading-relaxed text-muted mt-6">Drag a scheduled card to another day. Click any card to edit its caption, exact time or status.</p></>; }
