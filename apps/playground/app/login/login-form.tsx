...
export function LoginForm({ plan }: { plan?: "studio" | "agency" }) {
  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      const res = await fetch("/api/auth/magic-link", {
        ...
        body: JSON.stringify({ email, next: plan ? `/dashboard?checkout=${plan}` : "/dashboard" }),
      });
  ...
  return (
    <form onSubmit={submit} className="mt-7">
      ...
      <button className="btn w-full mt-3" disabled={busy}>{busy ? "Sending…" : selectedLabel(plan)}</button>
...
function selectedLabel(plan?: "studio" | "agency") {
  return plan ? `Email my link and continue →` : "Email my sign-in link →";
}
